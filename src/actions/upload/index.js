...
const login = (key) => {
  ...
  return axios
    ...
    .catch((e) => {
      ...
      exit(
        "Incorrect API key, not logged in to Metrological Dashboard or IP was not whitelisted"
      );
    });
};
...
const upload = async (metadata, user, apiKey, tgzFile) => {
  ...
  const { data } = await axios.post(
    `https://api.metrological.com/api/${user.type}/app-store/upload-lightning`,
    form,
    {
      headers,
    }
